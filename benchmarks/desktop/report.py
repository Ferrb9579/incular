...
native = read("results/windows/electron-ui-batched/incular-windows.json")
...
lines += [
    "",
    f"The latest Incular distribution bundle is **{sum(item['bytes'] for item in bundle) / 1e6:.2f} MB**, including the VC runtime DLL. Its memory has not been rebenchmarked. The retained Electron installed bundle is {electron['bundleBytes'] / 1e6:.2f} MB.",
    "",
