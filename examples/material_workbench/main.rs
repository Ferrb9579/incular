...
use incular::prelude::*;
use incular::widgets::HitTestBehavior;
use incular_material::prelude::*;
use incular_material::{
    Dialog as MaterialDialog, Drawer, NavigationDrawer, NavigationDrawerDestination,
...
fn main() {
    ...
    let dialog_dismiss = dialog_open.clone();
    let dialog = MaterialDialog::new(
        Column::new([
            ...
            Widget::from(FilledButton::tonal("Close dialog").on_click(move || {
                dialog_dismiss.set(false);
            })),
        ])
        .spacing(12.0),
    )
    ...
    let app = Application::new_with_options(options, move |_cx| {
        ...
        let dialog_is_open = dialog_open.get();
        ...
        let drawer_for_button = drawer_open.clone();
        let drawer_for_close = drawer_open.clone();
        ...
        .app_bar(
            AppBar::new(Text::new("Material Workbench"))
                .leading(DrawerButton::new().on_pressed(move || {
                    drawer_for_button.set(true);
                }))
        ...
        let app_content: Widget = if dialog_is_open {
            let dismiss = dialog_open.clone();
            let barrier = GestureDetector::new(Container::new().color(Color::rgba(0, 0, 0, 96)))
                .behavior(HitTestBehavior::Opaque)
                .on_tap(move || {
                    dismiss.set(false);
                });
            Stack::new([
                Widget::from(shell.clone()),
                Stack::new([
