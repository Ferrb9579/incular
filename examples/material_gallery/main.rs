...
use incular::material::{
    ...
    Dialog, Divider, DropdownButton, DropdownMenuItem, ElevatedButton, FilledButton, FilterChip,
    FloatingActionButton, InputChip, InputDecoration, LinearProgressIndicator, ListTile, Material,
    MaterialApp, MenuAnchor, MenuItemButton, NavigationBar, NavigationDestination,
    NavigationDrawer, NavigationDrawerDestination, OutlinedButton, PopupMenuButton, PopupMenuItem,
    RadioListTile, RangeSlider, RangeValues, Scaffold, ScaffoldMessenger,
    ScaffoldMessengerController, Slider, SnackBar, SnackBarAction, SwitchListTile, Tab, TabBar,
    TabBarView, TabController, TextButton, TextField, ThemeData, ThemeMode, Tooltip,
    TooltipController,
};
use incular::prelude::{
...
fn main() {
    ...
    let app = Application::new_with_options(options, move |_cx| {
        ...
        let menu_section = section(
            ...
            [
                Widget::from(
                    MenuAnchor::new(vec![
                        Widget::from(
                            MenuItemButton::label("New").on_pressed(|| println!("new menu item")),
                        ),
        ...
        let app_content: Widget = if dialog_open.get() {
            let dismiss = dialog_open.clone();
            let barrier = GestureDetector::new(
                Container::new().color(incular::prelude::Color::rgba(0, 0, 0, 96)),
            )
            .on_tap(move || {
                dismiss.set(false);
            });
            Stack::new([
                Widget::from(shell),
                Stack::new([
                    Widget::from(Positioned::fill(barrier)),
                    Widget::from(dialog.clone()),
                ])
                .into(),
            ])
            .into()
