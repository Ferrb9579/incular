...
| Ordinary scroll viewport attachment | Changed. One live viewport per `ScrollController`: a second live viewport laying out with the same controller now fails with `TreeError::DuplicateScrollAttachment` (carrying both viewport elements) before overwriting shared geometry, instead of last-layout-wins. The first viewport keeps its record and stays programmatically drivable. Cloned handles are the same attachment (scrollbars, app queries, and `FixedExtent` wrapping never claim); replacement swaps the driver with the old handle freed; unmount and tree drop release deterministically. Wheel, two-dimensional, and draggable paths are untouched. | W5; `crates/incular-widgets/tests/scroll_viewport_lists.rs` (duplicate rejection, replacement, unmount/remount, failed-update preservation, clones/scrollbar, deferred pre-attach jump, teardown release). |
