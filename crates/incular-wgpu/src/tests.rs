...
fn empty_and_degenerate_paths_are_safe_noop_meshes() {
    ...
    assert!(tessellate_path(&empty, FillRule::NonZero, None).is_none_or(|m| m.indices.is_empty()));
    let mut b = Path::builder();
    b.move_to(Offset::new(3., 3.)).line_to(Offset::new(3., 3.));
    let degenerate = b.build();
    assert!(
        tessellate_path(&degenerate, FillRule::NonZero, None).is_none_or(|m| m.indices.is_empty())
    );
}
#[test]
...
fn micro_normal_and_huge_masks_are_safe_and_warm() {
    ...
    let mut atlas = GlyphAtlas::new();
    for size in [
        4., 5., 6., 7., 8., 9., 10., 11., 12., 14., 16., 18., 20., 24., 32., 48., 64., 96., 128.,
...
fn counter_text_warms_the_atlas_incrementally() {
    ...
    eprintln!("counter atlas diagnostics: first={first:?}, click={first_click:?}, warm={warm:?}");
}
