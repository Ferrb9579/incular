...
use incular_widgets::{
    LengthLimitingTextInputFormatter, MaxLengthEnforcement, Stack, Text, TextInputFormatter, Widget,
};
use std::{
    cell::{Cell, RefCell},
...
fn rebuild_replaces_callbacks_and_unmount_releases_formatter() {
    ...
    let build = |limit, calls: Rc<Cell<usize>>| -> Widget {
        TextField::new(controller.clone())
            ...
            .on_changed(move |_| calls.set(calls.get() + 1))
            .into()
    };
    ...
    for _ in 0..4 {
        runtime
            .tree_mut()
            .update(root, Stack::new([build(1, calls.clone())]).into())
            .unwrap();
        runtime.run_frame(constraints).unwrap();
    }
    ...
    runtime
        ...
        .update(
            root,
            Stack::new([build(2, replacement_calls.clone())]).into(),
        )
        .unwrap();
    runtime.run_frame(constraints).unwrap();
    ...
    runtime
        .tree_mut()
        .update(root, Stack::new([Widget::from(Text::new("gone"))]).into())
        .unwrap();
